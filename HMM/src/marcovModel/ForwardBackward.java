...
public class ForwardBackward<O>  {
	...
	public BigDecimal AlphaTrans(int x,int train){
		...
		for(int y = 0;y<Model.getNumStates();y++){
			...
			temp=temp.add(alpha[y][train].multiply(Model.getA(y,x)));
		}
		return temp;
	}
	...
	public BigDecimal gamma(int i,int t){
		BigDecimal temp=new BigDecimal(0);
		temp=temp.add(alpha[i][t].multiply(beta[i][t]));
		temp=temp.divide(AlphaBeta(t),BigDecimal.ROUND_HALF_EVEN);
		return temp;
	}
	...
	public BigDecimal b(int i,int j,int t){
		BigDecimal temp=new BigDecimal(0);
		temp=temp.add(alpha[i][t].multiply(Model.getA(i,j).multiply(beta[j][t+1].multiply(
				Model.getEpsilon(j,trainingSet.get(t+1))))));
		BigDecimal adder=AlphaTransEpBeta(t);
		return temp.divide(adder,BigDecimal.ROUND_HALF_EVEN);
	}
