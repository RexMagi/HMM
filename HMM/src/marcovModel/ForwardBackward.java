...
public class ForwardBackward<O>  {
	...
	public ForwardBackward(ArrayList<O> trainingSet, HMM model) {
		...
		Model = model;
		sum= new BigDecimal[Model.getNumStates()];
			...
			for(int t=0;t<trainingSet.size();t++){
				if(t==0){
					...
					alpha[i][t]=Model.getPi(i).multiply
							(Model.getEpsilon(i,trainingSet.get(t)));
				}else {	
					...
					alpha[i][t]=Model.getEpsilon(i,trainingSet.get(t)).multiply(
							temp);
				}
				sum[i]=sum[i].add(alpha[i][t]);				
			} 
		for(int i=0;i<Model.getNumStates();i++){
			for(int t=0;t<trainingSet.size();t++){
				alpha[i][t]=alpha[i][t].divide(sum[i],MathContext.DECIMAL32);
			}
		}
	}
	public void back(){
		...
		for(int i=0;i<Model.getNumStates();i++)
			for(int t=0;t<trainingSet.size();t++){
				beta[i][t]=beta[i][t].divide(sum[i],MathContext.DECIMAL32);
			}
			...
			for(int j2 = 0; j2 < Model.getNumStates(); j2++){
				temp2=temp2.add(alpha[i2][t].multiply(
						Model.getA(i2,j2).multiply(
								Model.getEpsilon(j2,trainingSet.get(t+1)).multiply(
										beta[j2][t+1]))));
			}
